...
// void catch_signal(int s) {
//   pthread_mutex_lock(&buffer_prod_consumer.mutex);
//   pthread_cond_signal(&buffer_prod_consumer.cond);
//   pthread_mutex_unlock(&buffer_prod_consumer.mutex);
// }
  ...
  while(1) {
    ...
    switch(op) {
      case '1':
        ...
        req = read(fserv, name_client_req, sizeof(char)*40);
        resp = read(fserv, name_client_resp, sizeof(char)*40);
